...
public class MyGui extends JFrame {
	JPanel p; //<- 프레임
	JButton bt1, bt2, bt3;//Component <-부품 역할
	...
	public MyGui() {
		super(":MyGui Program v1.1::");//타이틀에 문자열이 올란간다
		p=new JPanel();
		add(p);//JPrame의 중앙에 판넬이 붙는다.
		p.setBackground(Color.lightGray);
		icon1=new ImageIcon("아이콘1.jpg");
		icon2=new ImageIcon("아이콘2.jpg");
		icon3=new ImageIcon("아이콘3.jpg");
		icon4=new ImageIcon("아이콘4.jpg");
		icon5=new ImageIcon("아이콘5.jpg");
		
		bt1=new JButton(icon1);
		
		bt2=new JButton("Home",icon2);
		bt2.setHorizontalTextPosition(JButton.CENTER);//텍스트 위치를 수평의 중아에 위치
		...
		tf2=new JTextField("단어를 입력하세요.",30);
		
		tf1.setBackground(Color.gray);
		
		tf2.setBackground(Color.black);//배경색
		...
		p.add(tf1);
		p.add(tf2);
		//JTwxtArea는 JScrollPane에 붙여야 스크롤바가 붙는다.
